...
| 10  | `Documents/PLAN_10_embedding.md`       | A       | stub pending  | `fredshell-core` embeddability contract per ADR 0006: `Shell` handle, async `Stream` of typed `ShellEvent`s, runtime-agnostic core, no fd writes from core. Owns PTY ownership model and the binary REPL refactor.      |
| 11  | `Documents/PLAN_11_parser.md`          | B       | stub pending  | In-house recursive-descent bash parser (`fredshell-parser`). Owns lexer, AST/CST, error recovery for incremental parsing, refusal taxonomy. Absorbs old 06b.1 (ADR 0005) and 06b.2 (lexer/parser scaffold).             |
...
| 21  | `Documents/PLAN_21_gettext.md`         | A       | stub pending  | Internationalisation strategy: `gettext`-style message catalogues for diagnostic messages, error explanations, and AI-facing prompts. Locale negotiation, fallback chain, catalogue compilation in build.               |
