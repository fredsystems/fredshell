...
> Last updated: 2026-06-28 — `PLAN_07` batch-1 handoff
> consolidation on `task-07/spec-drafting`, readying the branch for
> a PR (see `PLAN_07` §14 row 08.2-docs). `PLAN_07` now carries a
> §10 Status column plus a §10.1 batch-1 sheet checklist (5 of 10
> done: `cd`, `true`, `false`, `:`, `echo`; remaining `printf`,
> `set`, `shopt`, `unset`, `trap`) and a §15 Cleanup registry whose
> entry 08.2e-CU1 must be read before the `printf` sheet.
> `Documents/specs/README.md` gained the corpus-authoring runbook.
> Four stale `PLAN_16` milestone references in `PLAN_07` were
> corrected to `PLAN_19` (`PLAN_16` is config; `PLAN_19` is
> milestones).
>
> Earlier on 2026-06-28 — `PLAN_07` batch-1 drafting continued
> (08.2) on `task-07/spec-drafting`: the fifth sheet, `echo`,
> landed with 14 §3 rows (12 `support` backed by 12 hermetic corpus
> cases, 2 `defer` — `\u` / `\U` Unicode escapes to milestone 5,
> locale-dependent under the C-locale recorder; `xpg_echo`-default
> escapes to milestone 4 pending `shopt`). First moderate sheet of
> the batch. See `PLAN_07` §14 (08.2e). Remaining batch-1 sheets
> (`printf`, `set`, `shopt`, `unset`, `trap`) follow one per step,
> then 08.3 / 08.6 / 08.7 / 08.8 and the `pc` / `check` wiring of
> `check-specs`.
>
> Earlier on 2026-06-28 — `PLAN_07` batch-1 drafting continued
> (08.2) on `task-07/spec-drafting`: the fourth sheet, `:` (null
> command), landed with 6 hermetic corpus cases (all `support`; no
> wontfix / defer rows). First sheet to use the optional §7
> POSIX-divergence section and the first with a filename deviation
> (`colon.md`, by user decision, since a literal `:.md` is hostile
> to tooling). See `PLAN_07` §14 (08.2d).
>
> Earlier on 2026-06-28 — `PLAN_07` batch-1 drafting continued
> (08.2) on `task-07/spec-drafting`: the third sheet, `false`,
> landed with 4 hermetic corpus cases (all `support`; no wontfix /
> defer rows — the `true` mirror with a fixed exit status of `1`).
> See PLAN_07 §14 (08.2c).
>
> Earlier on 2026-06-28 — PLAN_07 batch-1 drafting continued
> (08.2) on `task-07/spec-drafting`: the second sheet, `true`,
> landed with 4 hermetic corpus cases (all `support`; no wontfix /
> defer rows). See PLAN_07 §14 (08.2b).
>
> Earlier on 2026-06-28 — PLAN_07 batch-1 drafting started
> (08.2) on `task-07/spec-drafting`: the first sheet, `cd`, landed
> with 7 hermetic corpus cases. Two pre-existing corpus/executor
> bugs surfaced and were fixed in their own commits (spec-record
> argv0 store-path; `cd` builtin global-CWD mutation + `eprintln!`
> from core). See PLAN_07 §14 (08.2a).
>
> Earlier on 2026-06-28 — PLAN_07 subtask 08.5 landed on
> `task-07/spec-drafting`: new `fredshell-spec-macros` proc-macro
> crate (the `refuse!` macro) plus the `fredshell_core::spec::Refusal`
> value type (compile-time row/classification validation; refusals
> are typed values rendered by the binary, never fd writes from core,
> per ADR 0006). Crate table in `AGENTS.md` updated. See PLAN_07 §14.
> Remaining: sheet batches (08.2 / 08.3 / 08.6), sheet-driven `help`
> (08.7), refusal corpus cases (08.8), and the `pc` / `check` wiring
> of `check-specs`.
>
> Earlier on 2026-06-28 — PLAN_07 implementation started on
> `task-07/spec-drafting`: row 07 status `drafted` → `in progress`.
> Subtasks 08.1 (spec-sheet template + `Documents/specs` README) and
> 08.4 (`cargo xtask check-specs` cross-reference checker) landed;
> see PLAN_07 §14. Remaining: sheet batches (08.2 / 08.3 / 08.6),
> the `refuse!` macro (08.5), and the `pc` / `check` wiring of
> `check-specs`.
>
> Earlier on 2026-05-24 — Cascade renumber to insert
...
| 07  | `Documents/PLAN_07_spec_drafting.md`   | A       | in progress   | Spec sheet template (one per builtin + feature), batch-of-10 review cadence, lint extensions tying `support` rows to corpus cases. 08.1 (template + README), 08.4 (`xtask check-specs`), 08.5 (`fredshell-spec-macros` + `refuse!`), 08.2a (`cd` sheet), 08.2b (`true` sheet), 08.2c (`false` sheet), 08.2d (`:` / `colon.md` sheet), and 08.2e (`echo` sheet) landed on `task-07/spec-drafting`; remaining batch-1 sheets (`printf`, `set`, `shopt`, `unset`, `trap`) pending — see `PLAN_07` §10.1 for the sheet checklist and §15 for surfaced cleanups. |
