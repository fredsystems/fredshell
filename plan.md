...
> Last updated: 2026-06-28 — PLAN_07 batch-1 drafting started
...
> from core). See PLAN_07 §14 (08.2a). Remaining batch-1 sheets
> (`echo`, `printf`, `true`, `false`, `:`, `set`, `shopt`, `unset`,
> `trap`) follow one per step, then 08.3 / 08.6 / 08.7 / 08.8 and
> the `pc` / `check` wiring of `check-specs`.
>
...
| 07  | `Documents/PLAN_07_spec_drafting.md`   | A       | in progress   | Spec sheet template (one per builtin + feature), batch-of-10 review cadence, lint extensions tying `support` rows to corpus cases. 08.1 (template + README), 08.4 (`xtask check-specs`), 08.5 (`fredshell-spec-macros` + `refuse!`), and 08.2a (`cd` sheet, 1st of batch 1) landed on `task-07/spec-drafting`; remaining batch-1 sheets pending. |
