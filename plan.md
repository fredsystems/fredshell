...
> See PLAN_07 §14 (08.2c). Remaining batch-1 sheets (`echo`,
> `printf`, `:`, `set`, `shopt`, `unset`, `trap`) follow one per
> step, then 08.3 / 08.6 / 08.7 / 08.8 and the `pc` / `check`
...
| 07  | `Documents/PLAN_07_spec_drafting.md`   | A       | in progress   | Spec sheet template (one per builtin + feature), batch-of-10 review cadence, lint extensions tying `support` rows to corpus cases. 08.1 (template + README), 08.4 (`xtask check-specs`), 08.5 (`fredshell-spec-macros` + `refuse!`), 08.2a (`cd` sheet), 08.2b (`true` sheet), and 08.2c (`false` sheet) landed on `task-07/spec-drafting`; remaining batch-1 sheets (`echo`, `printf`, `:`, `set`, `shopt`, `unset`, `trap`) pending. |
