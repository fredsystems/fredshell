...
//!
//! Future fields reserved for later subtasks: `stdin` (`PLAN_09a`),
...
pub enum CaseStatus {
    ...
    /// stored verbatim (e.g. `"PLAN_06b"`) so 05.5 / xtask compat
    /// can filter on it.
    Deferred(String),
}
...
mod tests {
    ...
    fn case_status_parse_known_values() {
        ...
        assert_eq!(
            CaseStatus::parse("deferred:PLAN_06b").unwrap(),
...
description = "meta"
script = "true\n"
status = "deferred:PLAN_06b"
tags = ["parameter-expansion", "posix-overlap"]
bash_version_min = "5.0"
"#,
        );
        let c = Case::load(&path).unwrap();
        assert_eq!(c.status, CaseStatus::Deferred("PLAN_06b".to_owned()));
