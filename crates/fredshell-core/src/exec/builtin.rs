...
//! today; the registry that consumes this trait lands with `PLAN_11`.
//! The shapes are nailed down now so:
//!
//! - `PLAN_11` can wire the dispatcher against a stable trait object.
//! - `PLAN_11` can implement individual builtins without re-litigating
//!   the calling convention.
//!
//! See `PLAN_11` §2.6 for the contract.
//!
//! ## v0 type choices
//!
//! `args` uses `&[String]` and `env` uses `&HashMap<String, String>`
//! to match [`crate::exec::ExecEnv`]'s v0 representation. `PLAN_11`
...
///
/// `#[non_exhaustive]` because `PLAN_11` adds further fields
...
pub struct Tier2Ctx<'a> {
    ...
    /// docs; `PLAN_11` migrates to `OsString`.
