...
> 08.5 landed; batch 1 is 5 of 10 sheets in — `cd`, `true`, `false`,
> `:`, `echo` done, with `printf`, `set`, `shopt`, `unset`, `trap`
> pending; batches 2–8, `help`, and refusal corpus cases pending).
> See §10.1 for the batch-1 checklist and §15 for surfaced cleanups.
...
| 08.2-rev | TBD | 2026-08-03 | PR #31 review round: addressed all 17 Copilot / CodeRabbit findings. **Sheet correctness (the important one):** `cd` §2 and row 3.7 stated `$CDPATH` precedence backwards — bash searches `$CDPATH` _before_ the plain relative interpretation, so a `$CDPATH` entry shadows `./dir`. Verified against bash 5.3p9 and corrected; the `cd_cdpath` case now stages competing `./target` and `$CDPATH/target` directories so it actually discriminates (it previously passed either way), and new row 3.13 plus `cd_cdpath_empty_entry` cover the empty-entry-means-cwd rule. Added §4 quirk 3 on the shadowing surprise. `echo` §7 overclaimed POSIX: POSIX makes `echo -n` _implementation-defined_ rather than requiring `-n` to be printed. `echo_empty` asserted `bytes=[]`, which `$(<file)` made vacuous (it passed whether echo wrote `\n` or nothing); it now probes `[ -s out ]` plus a builtin read-loop line count (`wc -c` is unavailable under `env_clear()`). **`check-specs` (08.4):** the row parser filtered _all_ empty cells, so a blank `Corpus` cell collapsed the row to 3 fields and was skipped silently, making the empty-corpus diagnostic unreachable and shifting columns when `Behaviour` was blank; it now strips only the pipe padding. Check 5 accepted any non-empty §6 body, so one paragraph satisfied every `defer` row; it now requires each row number to appear in §6 via a digit-boundary-aware `mentions_row` (so `3.13` does not satisfy `3.1`). 4 new tests; the loose fixture that exposed this was tightened. **`refuse!` (08.5):** `fredshell-core` lacked `extern crate self as fredshell_core`, so the first in-core `refuse!` would have failed with `E0433` — verified, fixed, and pinned by a regression test that exercises `refuse!` inside core. The expansion now emits `const _: &[u8] = include_bytes!(<sheet>)`, which finally makes §8.2's documented rebuild coupling real: touching `cd.md` now recompiles `fredshell-core` (verified; previously the sheet was not a tracked input at all, so stale validation could ship). Removed an `unsafe { std::env::set_var }` from `tests/ui.rs` — an AGENTS.md violation; confirmed by probe that cargo already puts the `build.rs` `rustc-env` in the test binary's environment and `trybuild`'s subprocess inherits it, so fixture isolation is preserved. Added the mandatory copyright header to all five `tests/ui/*.rs` fixtures and regenerated their `.stderr` snapshots. **Tests:** the `cd`/`env.cwd` test removed its temp directory _before_ the `canonicalize` that needed it, so the `unwrap_or` fallback always kept the raw `pwd` output; it now derives everything before cleanup and `expect`s the resolution. The argv0 regression test built its own `Command`, so it would have kept passing after the recorder stopped setting `$0`; both paths now share `reference_bash_command`, and removing the argument from production was confirmed to fail the test. It also no longer skips silently when `FREDSHELL_REFERENCE_BASH` is unset. **Docs:** stale `PLAN_16` milestone references corrected in `Documents/specs/README.md`, `_TEMPLATE.md` and `fredshell_core::spec`; the punctuation-named-builtin filename exception added to the README's layout section; the header status line now names the five landed sheets. One CodeRabbit rationale was inaccurate and is recorded here for the next reader: `cargo:rustc-env` does _not_ reach the rustc that expands a macro in a _consumer_ crate, so it is the test-binary environment (not the build script) that carries `FREDSHELL_SPECS_ROOT` into `trybuild`. Full suite green; `check-specs` still reports the expected 21 orphans and no sheet failures. |
