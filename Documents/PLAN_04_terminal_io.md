...
> Consumes: PLAN_02 §5, §6.1. Consumed by: PLAN_13 (line editor),
> PLAN_14 (prompt), PLAN_03 (capability boundary).
...
- **Key decoding.** Translating decoded CSI/SS3 sequences into
  semantic `KeyEvent`s belongs to PLAN_13.
- **Prompt rendering.** PLAN_14.
...
- **Mouse input.** Deferred. If/when the line editor enables it,
  decoding lives in PLAN_13; PLAN_04 only flips the DECSET bits.
...
the dialect looks like on the wire. `PLAN_13` owns _meaning_ once
...
3. Hand bytes to PLAN_13's key decoder.
4. PLAN_13 produces a `KeyEvent`.
...
6. Wire into the REPL: replace the current "read line from stdin"
   stub with a real `TerminalSession`-driven loop. PLAN_13 then
   builds on top.
...
- **PLAN_13** (line editor) consumes `wait()` and the raw-mode
  transition, and decodes the bytes PLAN_04's `input()` returns.
- **PLAN_14** (prompt) consumes `Capabilities` to decide which
