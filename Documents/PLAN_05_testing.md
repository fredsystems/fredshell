...
surface, Phase A landed and Phase B drafting), PLAN_10 (traps and job
...
regression; a case with `status = "deferred:PLAN_10"` matching is a
...
status = "deferred:PLAN_06"   # see §12 for the taxonomy
...
2. **Deferred breadth seed.** Write one positive case per category in
   ...
   `deferred:PLAN_06` becomes the single-bullet PR description for
...
with `status = "deferred:PLAN_06"` that suddenly matches is a signal
...
pub struct ExecEnv {
    ...
    // added by PLAN_06 Phase B; job-table slot filled in by PLAN_10.
}
...
- `cargo xtask compat --status deferred:PLAN_06` — restrict to a
...
looking references (PLAN_10 etc.) are intentional — the table
...
| `dirs`      | bash       | Vendor:Tier-1 | PLAN_10     | Directory stack.                                  |
| `disown`    | bash       | Vendor:Tier-1 | PLAN_10     | Job-table edit.                                   |
...
| `fg`        | POSIX      | Vendor:Tier-1 | PLAN_10     | Job control.                                      |
| `getopts`   | POSIX      | Vendor:Tier-1 | PLAN_10     | Argument parsing for scripts.                     |
| `hash`      | POSIX      | Vendor:Tier-1 | PLAN_10     | PATH lookup cache.                                |
| `help`      | bash       | Vendor:Tier-1 | PLAN_10     | Self-documenting; sourced from this table.        |
| `history`   | bash       | Vendor:Tier-1 | PLAN_07     | Owned by line editor.                             |
| `jobs`      | POSIX      | Vendor:Tier-1 | PLAN_10     | Job table.                                        |
| `kill`      | POSIX      | Vendor:Tier-1 | PLAN_10     | Signal delivery; also a coreutils binary.         |
| `let`       | bash       | Vendor:Tier-1 | PLAN_06     | Arithmetic eval; ties to `((...))`.               |
| `local`     | bash       | Vendor:Tier-1 | PLAN_06     | Scoped variable; function scope.                  |
| `logout`    | bash       | Vendor:Tier-1 | PLAN_10     | Login-shell-only exit.                            |
| `mapfile`   | bash       | Vendor:Tier-1 | PLAN_10     | Read into array; synonym `readarray`.             |
| `popd`      | bash       | Vendor:Tier-1 | PLAN_10     | Directory stack.                                  |
| `printf`    | POSIX      | Vendor:Tier-1 | PLAN_10     | Non-trivial format string parser.                 |
| `pushd`     | bash       | Vendor:Tier-1 | PLAN_10     | Directory stack.                                  |
| `pwd`       | POSIX      | Vendor:Tier-1 | PLAN_06     | Trivial; `-L` / `-P` flags.                       |
| `read`      | POSIX      | Vendor:Tier-1 | PLAN_10     | Line-editor implications; needs `ExecEnv::stdin`. |
| `readarray` | bash       | Vendor:Tier-1 | PLAN_10     | Synonym `mapfile`.                                |
...
| `suspend`   | bash       | Vendor:Tier-1 | PLAN_10     | Self-SIGTSTP.                                     |
| `test`      | POSIX      | Vendor:Tier-1 | PLAN_06     | Huge surface — file tests, string tests, etc.     |
| `times`     | POSIX      | Vendor:Tier-1 | PLAN_10     | Process times; thin libc.                         |
| `trap`      | POSIX      | Vendor:Tier-1 | PLAN_10     | Signal disposition table.                         |
| `true`      | POSIX      | Vendor:Tier-1 | PLAN_06     | Trivial.                                          |
| `type`      | POSIX      | Vendor:Tier-1 | PLAN_10     | Identify command kind; uses §11 table.            |
| `typeset`   | bash       | Vendor:Tier-1 | PLAN_06     | Synonym `declare`.                                |
| `ulimit`    | POSIX      | Vendor:Tier-1 | PLAN_10     | `setrlimit` wrapper.                              |
| `umask`     | POSIX      | Vendor:Tier-1 | PLAN_10     | `umask` syscall wrapper.                          |
| `unalias`   | bash       | Vendor:Tier-1 | PLAN_06     | Alias-table edit.                                 |
| `unset`     | POSIX      | Vendor:Tier-1 | PLAN_06     | Variable / function removal.                      |
| `wait`      | POSIX      | Vendor:Tier-1 | PLAN_10     | Wait for jobs/pids.                               |
...
`deferred:PLAN_06` case for that feature flips to a `RECLASSIFY`
...
`cargo xtask compat --status deferred:PLAN_06` lists every case
PLAN_06 is responsible for. The list is the literal task inventory
for that document. When the list is empty (every `deferred:PLAN_06`
...
- Any actual implementation of builtins or executor features from
  §11. Owned by PLAN_06, PLAN_10, PLAN_06, PLAN_06.
