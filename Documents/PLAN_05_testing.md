...
> Last updated: 2026-05-21 — 05.12 close-out: status flipped to
...
surface, Phase A landed and Phase B drafting), PLAN_12 (traps and job
control), and PLAN_15 (milestones).
...
regression; a case with `status = "deferred:PLAN_12"` matching is a
...
status = "deferred:PLAN_11"   # see §12 for the taxonomy
...
2. **Deferred breadth seed.** Write one positive case per category in
   ...
   `deferred:PLAN_11` becomes the single-bullet PR description for
...
with `status = "deferred:PLAN_11"` that suddenly matches is a signal
...
pub struct ExecEnv {
    ...
    // added by PLAN_06 Phase B; job-table slot filled in by PLAN_12.
}
...
editor to exist. Its design is owned by PLAN_13 (interactive UX)
...
- `cargo xtask compat --status deferred:PLAN_11` — restrict to a
...
looking references (PLAN_12 etc.) are intentional — the table
...
| `dirs`      | bash       | Vendor:Tier-1 | PLAN_12     | Directory stack.                                  |
| `disown`    | bash       | Vendor:Tier-1 | PLAN_12     | Job-table edit.                                   |
...
| `fc`        | bash       | Vendor:Tier-1 | PLAN_13     | History subsystem; owned by line editor.          |
| `fg`        | POSIX      | Vendor:Tier-1 | PLAN_12     | Job control.                                      |
| `getopts`   | POSIX      | Vendor:Tier-1 | PLAN_12     | Argument parsing for scripts.                     |
| `hash`      | POSIX      | Vendor:Tier-1 | PLAN_12     | PATH lookup cache.                                |
| `help`      | bash       | Vendor:Tier-1 | PLAN_12     | Self-documenting; sourced from this table.        |
| `history`   | bash       | Vendor:Tier-1 | PLAN_13     | Owned by line editor.                             |
| `jobs`      | POSIX      | Vendor:Tier-1 | PLAN_12     | Job table.                                        |
| `kill`      | POSIX      | Vendor:Tier-1 | PLAN_12     | Signal delivery; also a coreutils binary.         |
| `let`       | bash       | Vendor:Tier-1 | PLAN_06     | Arithmetic eval; ties to `((...))`.               |
| `local`     | bash       | Vendor:Tier-1 | PLAN_06     | Scoped variable; function scope.                  |
| `logout`    | bash       | Vendor:Tier-1 | PLAN_12     | Login-shell-only exit.                            |
| `mapfile`   | bash       | Vendor:Tier-1 | PLAN_12     | Read into array; synonym `readarray`.             |
| `popd`      | bash       | Vendor:Tier-1 | PLAN_12     | Directory stack.                                  |
| `printf`    | POSIX      | Vendor:Tier-1 | PLAN_12     | Non-trivial format string parser.                 |
| `pushd`     | bash       | Vendor:Tier-1 | PLAN_12     | Directory stack.                                  |
| `pwd`       | POSIX      | Vendor:Tier-1 | PLAN_06     | Trivial; `-L` / `-P` flags.                       |
| `read`      | POSIX      | Vendor:Tier-1 | PLAN_12     | Line-editor implications; needs `ExecEnv::stdin`. |
| `readarray` | bash       | Vendor:Tier-1 | PLAN_12     | Synonym `mapfile`.                                |
...
| `suspend`   | bash       | Vendor:Tier-1 | PLAN_12     | Self-SIGTSTP.                                     |
| `test`      | POSIX      | Vendor:Tier-1 | PLAN_06     | Huge surface — file tests, string tests, etc.     |
| `times`     | POSIX      | Vendor:Tier-1 | PLAN_12     | Process times; thin libc.                         |
| `trap`      | POSIX      | Vendor:Tier-1 | PLAN_12     | Signal disposition table.                         |
| `true`      | POSIX      | Vendor:Tier-1 | PLAN_06     | Trivial.                                          |
| `type`      | POSIX      | Vendor:Tier-1 | PLAN_12     | Identify command kind; uses §11 table.            |
| `typeset`   | bash       | Vendor:Tier-1 | PLAN_06     | Synonym `declare`.                                |
| `ulimit`    | POSIX      | Vendor:Tier-1 | PLAN_12     | `setrlimit` wrapper.                              |
| `umask`     | POSIX      | Vendor:Tier-1 | PLAN_12     | `umask` syscall wrapper.                          |
| `unalias`   | bash       | Vendor:Tier-1 | PLAN_06     | Alias-table edit.                                 |
| `unset`     | POSIX      | Vendor:Tier-1 | PLAN_06     | Variable / function removal.                      |
| `wait`      | POSIX      | Vendor:Tier-1 | PLAN_12     | Wait for jobs/pids.                               |
...
`deferred:PLAN_11` case for that feature flips to a `RECLASSIFY`
...
`cargo xtask compat --status deferred:PLAN_11` lists every case
PLAN_06 is responsible for. The list is the literal task inventory
for that document. When the list is empty (every `deferred:PLAN_11`
...
- Tier-2 oils-spec fetching and translation. Owned by PLAN_05's
  ...
  exit criteria).
- Tier-3 real-world script selection. Owned by PLAN_15 (milestones).
- L4 PTY harness. Owned by PLAN_13.
...
- Any actual implementation of builtins or executor features from
  §11. Owned by PLAN_06, PLAN_12, PLAN_06, PLAN_06.
