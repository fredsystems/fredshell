...
argument `-` it changes to `$OLDPWD` and prints the new directory. A
relative `dir` that is not found relative to `$PWD` is searched for in
the colon-separated directories named by `$CDPATH`, unless `dir`
begins with a slash.
...
| 3.7  | `cd` resolves a relative target through `$CDPATH` when not found relative to `$PWD`   | support        | `builtins_tier1/cd_cdpath.case.toml`        |
...
3. **Too-many-arguments is a usage error, not a no-op.** `cd a b`
