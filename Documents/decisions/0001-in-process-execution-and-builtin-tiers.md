...
2. **Commands are dispatched through a tiered builtin model:**
   - **Tier 1 — Shell builtins.** Mandatory in-process; POSIX requires them to
     ...
     The exact inventory is owned by `PLAN_06_exec.md`.
...
- **Risk: tier-2 sprawl.** Every "we should also build `cp` and `mv`" request
  ...
  **Mitigation:** new tier-2 entries require an ADR or a documented entry in
  `PLAN_06_exec.md` (Phase B inventory) with usage-frequency justification.
...
- `PLAN_06_exec.md` — the executor, the Tier-1 builtin inventory, and
  the native parser/executor that this ADR presupposes (Phase A
  skeleton landed; Phase B compat work pending).
